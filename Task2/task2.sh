#!/bin/bash
# Task 2: Secure Symmetric Key Exchange using OpenSSL

# Step 1: Generate a recipient RSA key pair
...
# Step 2: Generate a random 256-bit symmetric key (binary)
openssl rand -out symmetric_key.bin 32
echo "Generated random 256-bit symmetric key (symmetric_key.bin)"

# Step 3: Encrypt the symmetric key using the recipient's RSA public key
...
# Step 4: Recipient decrypts the symmetric key using their RSA private key
...
# Step 5: Encrypt a message using the shared symmetric key (AES-256-CBC)
